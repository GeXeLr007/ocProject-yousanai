...
public class PoralController {
    ...
    @ApiOperation(value = "推荐接口接口", notes = "推荐区接口的接口")
    @PostMapping(value = "/recommond")
    public IMoocJSONResult recommond(Integer count) {
        CourseQueryDto queryEntity = new CourseQueryDto();
        queryEntity.setCount(2);//4门
        queryEntity.setFree(true);
        queryEntity.descSortField("weight");//按照weight降序排列
        List<TCourse> actionCourseList = tCourseService.queryList(queryEntity);
